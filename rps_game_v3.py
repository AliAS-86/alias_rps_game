...
class RpcGame:
    ...
    def get_game_mode(self):
        ...
        while not is_valid:
            ...
            if not validation:
                ...
                self.game_announcement("Invalid input, please try again")
    ...
    def get_num_of_games(self):
        ...
        while not is_valid:  
            try:
                ...
                if user_input <= 0:
                    self.game_announcement("Invalid input, please enter a positive number that is 1 or higher or q to exit")
                # print(f"user_input: {user_input}, input type: {type(user_input)}")
                else:
                    is_valid = True
            except ValueError:
                self.game_announcement("Invalid input, please try again by entering a number")
        ...
        while game_counter <= games:
            ...
            while round_counter <= rounds:
                ...
                if round_result == player_1:
                    player_1_score += 1
                    self.game_announcement(f"current score now: {player_1}: {player_1_score} / {player_2}: {player_2_score}")
            ...
            self.game_announcement(f"All {rounds} reound(s) of game {game_counter} of {games} are done...")
            self.game_announcement(f"Game {game_counter} of {games}:\nRound winner: {round_winner}\nGame standings:\n{player_1}: {player_1_total_score}\n{player_2}: {player_2_total_score}")
            round_counter = 1
            game_counter += 1
            player_1_score = 0
            player_2_score = 0
    ...
    def get_play(self, player):
        ...
        while not is_valid:
            ...
            if not validation:
                self.game_announcement("Invalid play, please retry by entering rock, paper, or scissor")
            elif validation:    
                self.game_announcement(f"{player} picked {play}")
                is_valid = True
        return play
    ...
    def get_auto_play(self, player):
        ...
        self.game_announcement(f"{player} picked {pick}")
        return pick
